        ...
        <div class="leftside-navigation">
            <ul class="sidebar-menu" id="nav-accordion">
                ...
                <li class="sub-menu">
                    ...
                    <ul class="sub">
                        <li><a href="{{URL::to('/manage-banner')}}">Quản lí slider</a></li>           
                    </ul>
                ...
                <li class="sub-menu">
                    ...
                    <ul class="sub">
                        <li><a href="{{URL::to('/insert-coupons')}}">Quản lí mã giảm giá</a></li>
                        <li><a href="{{URL::to('/list-coupons')}}">Liệt kê mã giảm giá</a></li>
...
<script src="{{asset('back-end/js/jquery.form-validator.min.js')}}"></script>


<script type="text/javascript">
    {{-- click cap nhat update_quantity_order --}}
    $('.update_quantity_order').click(function(){
        // alert('đã click');
        //  var order_qty = $('.order_qty_'+ product_id).val(); product_id lấy theo var product_id dòng 211
        ...
        // alert(order_code);
         $.ajax({
            url: '{{url('/update-qty')}}',
            method: 'post',
            data: {_token: _token, product_id: product_id, order_qty: order_qty, order_code: order_code},
...
{{-- change Xác định một thành phần đã được thay đổi.
         --}}
{{-- $(this) lấy value trong class order_detalis --}}
<script type="text/javascript">
    $('.order_details').change(function(){
        ...
        // lấy tất số lượng đẩy và mảng quantity
        let quantity = [];
        ...
        let flag = 0;
        // lấy product_sales_quantity và product_quantity
        for(let i = 0; i < order_product_id.length; i++){
            // order_qty số lượng sản phẩm khách đặt mua trong hóa đơn
           let order_qty = $('.order_qty_' + order_product_id[i]).val();
           // product_quantity số lượng sản phẩm trong kho
           let product_quantity = $('.order_qty_storage_' + order_product_id[i]).val();
           // alert(order_qty);
           // alert(product_quantity);
           if(Number(order_qty) > Number(product_quantity)){
                // alert('')
                flag++;
                if(flag == 1){
        ...
        if(flag == 0) {
            ...
            method: 'post',
            data: {_token: _token, order_id: order_id, order_status: order_status, quantity: quantity, order_product_id: order_product_id},
            success: function(data){
